            ...
            <v-card class="elevation-12">
              ...
              <v-card-text>
                  ...
                  <v-text-field prepend-icon="email" name="email"
                   ...
                   v-model="User.email"
                   type="email"
                   :rules="[emailIsDuplicate]" required></v-text-field>
...
export default {
  ...
  computed: {
    ...
    emailIsDuplicate () {
      if (this.User.email.trim()) {
        var emailHasDuplicate = this.$store.getters['Users/getDuplicateEmail'](this.User.email.trim())
        return emailHasDuplicate && emailHasDuplicate.id !== this.User.id ? 'Email is in used' : null
      }
      return true
    }
  },
  methods: {
    onSignup () {
      this.$store.dispatch('Users/onSigningUpChange', this.User)
