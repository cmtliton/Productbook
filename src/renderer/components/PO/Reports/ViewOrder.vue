      ...
      <v-dialog v-model="dialog" width="384px">
        <v-card id="Printarea">
          <v-card-text>
            ...
            <p class="text-xs-center pa-0 ma-0">Date: {{Order.PODate | date}}</p>
            <v-divider></v-divider>
            <p class="text-xs-left pa-0 ma-0"><b>Order No:</b> {{Order.id}}</p>
          </font>
            <v-divider></v-divider>
            <div>
              <font size="1" face="Time new Roman">
            <table align="center" width="100%">
              ...
              <tr v-for="(product, index) in POitems" :key="product.ProductId">
              ...
              <tr>
                <td></td>
                <th class="text-xs-right">Order Total</th>
          ...
          <v-card-actions @click="print('Printarea')">
            <v-spacer></v-spacer>
            <p><font face="Time new Roman" size="1">Order By {{User}}</font></p>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-layout>
...
export default {
  ...
  computed: {
    Order () {
      return this.$store.getters['PurOrds/getPurOrds'].find(p => p.id === this.PON)
    },
    POitems () {
      var sl = 1
      return this.Order.POitems.map(item => {
    ...
    NetTotal () {
      return this.Order.POCartTotal
    },
    NetTotalqty () {
      return this.Order.POCartTotalqty
    },
    User () {
      return this.$store.getters['Users/getUsers'].find(user => user.username.trim() === this.Order.CreatorId.trim()).fullname
    },
    Suppliers () {
      return this.$store.getters['Suppliers/getSuppliers']
    },
    SupplierId () { // Joining Order and PO For gettting SupplierID
      return this.$store.getters['PurOrds/getPurOrds'].find(pon => pon.id === this.PON).SupplierId
    },
    SupplierInfo () {
      return this.Suppliers.find(supplier => supplier.id === this.SupplierId)
    },
    Payment () {
      return this.$store.getters['PurOrdRcvs/getSupplilerPaymentHistory'].find(pmt => pmt.PRN === this.Order.id)
    },
    getCompany () {
  ...
  methods: {
    ...
    print (Printarea) {
      ...
      var pageContent = function () {
        ...
        doc.text('Date: ' + vm.Order.PODate, pageWidth / 2, pageHeight - 252, 'center')
        doc.line(10, 47, pageWidth - 15, 47)
        doc.text(10, 51, 'Order No: ' + vm.Order.id)
        doc.line(10, 53, pageWidth - 15, 53)
      }
      doc.autoTable(columns, vm.POitems, {
      ...
      doc.setFontType('normal')
      doc.setFontSize(12)
      doc.text(pageWidth / 2 - 10, finalY + 5, 'Order Total' + '       ' + vm.NetTotalqty + '                                    ' + vm.NetTotal)
      doc.line(pageWidth / 2 + 10, finalY + 25, pageWidth - 15, finalY + 25)
      doc.text('Order By ' + vm.User, pageWidth / 2 + 50, finalY + 30, 'center')
      doc.save(vm.Order.id + '.pdf')
    },
    close () {
      this.dialog = false
