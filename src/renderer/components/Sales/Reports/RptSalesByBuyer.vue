  ...
  computed: {
    ...
    PrintProductSummary () {
      ...
      return this.SalesProductSummary.map(SalesPrd => {
        var product = this.$store.getters['Products/getProductsWithItemAndBrand'].find(prd => prd.ProductId === SalesPrd.ProductId)
        return {
          Sl: sl++,
          ...
          MRP: SalesPrd.MRP,
          TPrice: SalesPrd.quantity * SalesPrd.MRP
        }
      })
    },
    ...
    printInvoice (Printarea) {
      ...
      var pageContent = function () {
        ...
        if (vm.BuyerInfo !== undefined) {
          ...
          doc.text('Sales Buyer Wise from ' + vm.datereport.d1.toISOString() + ' to ' + vm.datereport.d2.toISOString(), pageWidth / 2, pageHeight - 252, 'center')
        } else {
          doc.text('Sales Summary from ' + vm.datereport.d1.toISOString() + ' to ' + vm.datereport.d2.toISOString(), pageWidth / 2, pageHeight - 252, 'center')
        }
